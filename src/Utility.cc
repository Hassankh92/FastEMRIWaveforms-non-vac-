                              ...
                              double a, double p, double e, double x)
{
    ...
    *CapitalUpsilonTheta_ = CapitalUpsilonTheta;
    *CapitalUpsilonr_ = CapitalUpsilonr;
    // omegaphi = CapitalUpsilonPhi / CapitalGamma;
}
                              ...
                              double a, double p, double e, double x)
{
    ...
    if ((e==0.0) && (x == 1.0))
    {
        *OmegaR_ = (sqrt(2*a + (-3 + p)*sqrt(p))*sqrt((p*(-2*pow(a,2) + 6*a*sqrt(p) + (-5 + p)*p + (pow(a - sqrt(p),2)*(pow(a,2) - 4*a*sqrt(p) - (-4 + p)*p))/abs(pow(a,2) - 4*a*sqrt(p) - (-4 + p)*p)))/(2*a*sqrt(p) + (-3 + p)*p)))/(pow(p,1.25)*(a + pow(p,1.5)));
        *OmegaTheta_ = (sqrt(2*a + (-3 + p)*sqrt(p))*abs((pow(p,0.25)*sqrt(3*pow(a,2) - 4*a*sqrt(p) + pow(p,2)))/sqrt(2*a + (-3 + p)*sqrt(p))))/(pow(p,1.25)*(a + pow(p,1.5)));
        *OmegaPhi_ = 1/(a + pow(p,1.5));
    }
    else
    {
