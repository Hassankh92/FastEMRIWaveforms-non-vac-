...
__deriv__ void pn5_base_func(double *pdot, double *edot, double *Ydot,
                      ...
                      double epsilon, double a, double p, double e, double x, double* additional_args)
{
  ...
  //  /*Disk effect */
    ...
    double dL_dp = (-3*Power(a,3) + Power(a,2)*(8 - 3*p)*Sqrt(p) + (-6 + p)*Power(p,2.5) + 3*a*p*(-2 + 3*p))/(2.*Power(2*a + (-3 + p)*Sqrt(p),1.5)*Power(p,1.75));
    *pdot =  -(Ldot_Kerr + Ldot_disk  )/dL_dp;
                  ...
                  double epsilon, double a, double p, double e, double x, double* additional_args)
{
 ...
 if(alpha==0){
    ...
    double Edot_Kerr = (interps->Edot_Kerr->eval(u, a)*pow(yPN,6.) + EdotPN(p,a));
    double Edot = epsilon*(Edot_Kerr + cloud_horizon_flux_switch * cloud_Edot_Hor + cloud_Edot_Inf) ;
...
CloudKerrCircFlux::~CloudKerrCircFlux()
{
    ...
    ODECarrier::ODECarrier(std::string func_name_, std::string few_dir_)
    {
        ...
        few_dir = few_dir_;
            ...
            else if (func_name == "KerrCircFlux")
            {
        
                KerrCircFlux* temp = new KerrCircFlux(few_dir);
            ...
            else if (func_name == "MigTorqKerrCircFlux")
            {
        
                MigTorqKerrCircFlux* temp = new MigTorqKerrCircFlux(few_dir);
            ...
            else if (func_name == "CloudKerrCircFlux")
            {
        
                CloudKerrCircFlux* temp = new CloudKerrCircFlux(few_dir);
                      ...
                      double epsilon, double a, double p, double e, double Y, double* additional_args)
    {
            ...
            else if (func_name == "KerrCircFlux")
            {
                ...
                KerrCircFlux* temp = (KerrCircFlux*)func;
            ...
            else if (func_name == "MigTorqKerrCircFlux")
            {
                ...
                MigTorqKerrCircFlux* temp = (MigTorqKerrCircFlux*)func;
            ...
            else if (func_name == "CloudKerrCircFlux")
            {
                ...
                CloudKerrCircFlux* temp = (CloudKerrCircFlux*)func;
